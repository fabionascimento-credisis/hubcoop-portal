  ...
  return (
    <button
      ...
      className={`
        ...
        ${
          active
            ? 'bg-gray-800 font-semibold shadow-md border-l-4 border-white' // Alterado para cinza escuro (bg-gray-800)
...
// =======================================================================
// 9. Componente Modal de Solicitação de Cartão (ATUALIZADO)
// =======================================================================
function ModalSolicitarCartao({ cooperado, onClose }: { cooperado: Cooperado; onClose: () => void }) {
  const [step, setStep] = useState(1);
  const [selectedCardId, setSelectedCardId] = useState('');
  
  // Estado para os Serviços Adicionais
  ...
  // Mock local para exibição visual dos cartões neste modal
  const mockTiposDeCartao = [
    { id: 'classic', nome: 'Visa Classic', img: 'https://placehold.co/100x60/silver/white?text=Classic' },
    { id: 'gold', nome: 'Visa Gold', img: 'https://placehold.co/100x60/gold/white?text=Gold' },
    { id: 'platinum', nome: 'Visa Platinum', img: 'https://placehold.co/100x60/e5e4e2/333?text=Platinum' },
    { id: 'empresarial', nome: 'Visa Empresarial', img: 'https://placehold.co/100x60/3e4e5e/white?text=Empresa' },
    { id: 'infinite', nome: 'Visa Infinite', img: 'https://placehold.co/100x60/111/white?text=Infinite' },
  ];

  const proximaEtapa = () => setStep((s) => s + 1);
  const etapaAnterior = () => setStep((s) => s - 1);

  const toggleServico = (key: 'ppr' | 'sms' | 'whatsapp') => {
    setServicos(prev => ({ ...prev, [key]: !prev[key] }));
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl p-8 max-h-[90vh] flex flex-col animate-fade-in">
        <div className="flex justify-between items-center pb-4 border-b">
          <h2 className="text-2xl font-semibold text-gray-800">Solicitação de Cartão</h2>
          <button onClick={onClose}><X className="w-6 h-6 text-gray-400 hover:text-red-500" /></button>
        </div>

        {/* Indicador de Passos */}
        <div className="flex items-center justify-center space-x-4 my-6">
           <div className={`flex items-center space-x-2 ${step >= 1 ? 'text-gray-800 font-bold' : 'text-gray-400'}`}>
             <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 1 ? 'border-gray-800 bg-gray-800 text-white' : 'border-gray-300'}`}>1</div>
             <span>Produto</span>
           </div>
           <div className="w-12 h-0.5 bg-gray-300"></div>
           <div className={`flex items-center space-x-2 ${step >= 2 ? 'text-gray-800 font-bold' : 'text-gray-400'}`}>
             <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 2 ? 'border-gray-800 bg-gray-800 text-white' : 'border-gray-300'}`}>2</div>
             <span>Serviços</span>
           </div>
           <div className="w-12 h-0.5 bg-gray-300"></div>
           <div className={`flex items-center space-x-2 ${step >= 3 ? 'text-gray-800 font-bold' : 'text-gray-400'}`}>
             <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 3 ? 'border-gray-800 bg-gray-800 text-white' : 'border-gray-300'}`}>3</div>
             <span>Confirmar</span>
           </div>
        </div>

        <div className="flex-1 overflow-y-auto pr-2 p-2">
          
          {/* PASSO 1: ESCOLHA DO PRODUTO */}
          {step === 1 && (
            <div className="animate-fade-in">
               <h3 className="text-lg font-semibold mb-4 text-gray-700">Escolha o Produto</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {mockTiposDeCartao.map((card) => (
                    <button 
                      key={card.id} 
                      onClick={() => setSelectedCardId(card.id)} 
                      className={`flex items-center p-4 border rounded-lg transition-all hover:bg-gray-50 ${
                        selectedCardId === card.id 
                          ? 'ring-2 ring-gray-600 border-gray-600 bg-gray-100' 
                          : 'border-gray-200'
                      }`}
                    >
                       <img src={card.img} alt={card.nome} className="w-16 mr-4 rounded shadow-sm" />
                       <span className="font-semibold text-gray-800">{card.nome}</span>
                       {selectedCardId === card.id && <CheckCircle className="w-5 h-5 text-gray-800 ml-auto" />}
                    </button>
                  ))}
               </div>
            </div>
          ...
          {/* PASSO 2: SERVIÇOS ADICIONAIS (NOVO) */}
          {step === 2 && (
            <div className="animate-fade-in space-y-4">
               <div>
                  <h3 className="text-lg font-semibold text-gray-700">Serviços Adicionais</h3>
                  <p className="text-sm text-gray-500">Selecione os serviços opcionais para este cartão.</p>
               </div>
               
               {/* Opção PPR */}
               <label className={`flex items-start p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${servicos.ppr ? 'border-gray-600 bg-gray-50 ring-1 ring-gray-400' : 'border-gray-200'}`}>
                  <div className="flex items-center h-5">
                    <input 
                      type="checkbox" 
                      checked={servicos.ppr} 
                      onChange={() => toggleServico('ppr')} 
                  ...
                  <div className="ml-3 w-full">
                     <span className="font-semibold text-gray-800 flex items-center justify-between">
                       <span className="flex items-center"><ShieldAlert className="w-4 h-4 mr-2 text-gray-600"/> Seguro Perda e Roubo (PPR)</span>
                       <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded">Recomendado</span>
                     </span>
                     <span className="block text-sm text-gray-500 mt-1">
                       Proteção completa contra transações indevidas em caso de perda, roubo ou furto do cartão.
                     </span>
                  </div>
               </label>

               {/* Opção SMS */}
               <label className={`flex items-start p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${servicos.sms ? 'border-gray-600 bg-gray-50 ring-1 ring-gray-400' : 'border-gray-200'}`}>
                  <div className="flex items-center h-5">
                    <input 
                      type="checkbox" 
                      checked={servicos.sms} 
                      onChange={() => toggleServico('sms')} 
                      className="w-5 h-5 text-gray-800 border-gray-300 rounded focus:ring-gray-600" 
                    />
                  </div>
                  <div className="ml-3">
                     <span className="block font-semibold text-gray-800 flex items-center">
                        <Smartphone className="w-4 h-4 mr-2 text-gray-600"/> Notificações via SMS
                     </span>
                     <span className="block text-sm text-gray-500 mt-1">
                       Receba alertas em tempo real no seu celular a cada compra realizada.
                     </span>
                  </div>
               </label>

               {/* Opção WhatsApp */}
               <label className={`flex items-start p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${servicos.whatsapp ? 'border-gray-600 bg-gray-50 ring-1 ring-gray-400' : 'border-gray-200'}`}>
                  <div className="flex items-center h-5">
                    <input 
                      type="checkbox" 
                      checked={servicos.whatsapp} 
                      onChange={() => toggleServico('whatsapp')} 
                      className="w-5 h-5 text-gray-800 border-gray-300 rounded focus:ring-gray-600" 
                    />
                  </div>
                  <div className="ml-3">
                     <span className="block font-semibold text-gray-800 flex items-center">
                        <div className="w-4 h-4 mr-2 bg-green-500 rounded-full flex items-center justify-center text-white text-[10px]">W</div>
                        Notificações via WhatsApp
                     </span>
                     <span className="block text-sm text-gray-500 mt-1">
                       Receba sua fatura digital e avisos de segurança diretamente no WhatsApp oficial.
                     </span>
                  </div>
               </label>
            </div>
          ...
          {step === 3 && (
            <div className="flex flex-col items-center justify-center py-4 animate-fade-in text-center">
               ...
               <h3 className="text-2xl font-semibold text-gray-800">Confirmar Solicitação?</h3>
               <p className="text-gray-600 mt-2 max-w-md">
                 Confira os dados abaixo antes de enviar para produção.
               </p>
               
               <div className="mt-6 w-full max-w-md bg-gray-50 border rounded-lg overflow-hidden text-left">
                 <div className="p-4 border-b">
                   <p className="text-xs text-gray-500 uppercase">Cooperado</p>
                   <p className="font-bold text-gray-800">{cooperado.nome}</p>
                 </div>
                 <div className="p-4 border-b flex justify-between">
                    <div>
                       <p className="text-xs text-gray-500 uppercase">Produto</p>
                       <p className="font-semibold text-gray-800">{mockTiposDeCartao.find(c => c.id === selectedCardId)?.nome}</p>
                    </div>
                    <CreditCard className="w-6 h-6 text-gray-400"/>
                 </div>
                 <div className="p-4 bg-white">
                   <p className="text-xs font-bold text-gray-500 uppercase mb-2">Serviços Contratados</p>
                   <ul className="space-y-2 text-sm">
                     <li className="flex items-center text-gray-700">
                       {servicos.ppr ? <Check className="w-4 h-4 text-green-500 mr-2"/> : <X className="w-4 h-4 text-red-300 mr-2"/>}
                       Seguro PPR
                     </li>
                     <li className="flex items-center text-gray-700">
                       {servicos.sms ? <Check className="w-4 h-4 text-green-500 mr-2"/> : <X className="w-4 h-4 text-red-300 mr-2"/>}
                       Notificações SMS
                     </li>
                     <li className="flex items-center text-gray-700">
                       {servicos.whatsapp ? <Check className="w-4 h-4 text-green-500 mr-2"/> : <X className="w-4 h-4 text-red-300 mr-2"/>}
        ...
        <div className="flex justify-between pt-6 border-t mt-2">
           <button 
             onClick={step === 1 ? onClose : etapaAnterior} 
             className="px-6 py-2 text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 font-medium"
           >
           ...
           {step < 3 ? (
              <button 
                ...
                className="px-6 py-2 text-white bg-gray-800 rounded-lg hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium shadow-sm"
              >
                Continuar
              </button>
           ) : (
              <button 
                onClick={onClose} 
                className="px-8 py-2 text-white bg-green-600 rounded-lg hover:bg-green-700 font-medium shadow-md flex items-center"
              >
                <Check className="w-5 h-5 mr-2" /> Finalizar Pedido
              </button>
           )}
        </div>
      </div>
